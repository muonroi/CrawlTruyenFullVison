...
from config.config import BASE_URLS, COMPLETED_FOLDER, DATA_FOLDER, LOADED_PROXIES, PROXIES_FILE, PROXIES_FOLDER, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID
...
@router.message(F.text == '/startcrawl')
async def handle_start_crawl(message: types.Message):
    if str(message.chat.id) != TELEGRAM_CHAT_ID:
        await message.reply("Không có quyền chạy crawler.")
        return
    await message.reply("Đã nhận lệnh, bắt đầu crawl ...")
    try:
        await send_telegram_notify("🔄 Đang chạy crawl kiểm tra missing qua Telegram...")
        await loop_once_multi_sites(force_unskip=False)
        await send_telegram_notify("✅ DONE: Crawl kiểm tra missing xong toàn bộ (triggered từ Telegram).")
    except Exception as e:
        await send_telegram_notify(f"❌ Crawler lỗi: {e}")
