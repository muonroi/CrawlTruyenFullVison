...
async def crawl_all_sources_until_full(
    ...
    num_batches=10, state_file=None, adapter: BaseSiteAdapter = None # type: ignore
):
        ...
        for source in sources:
            ...
            try:
                chapters = await adapter.get_chapter_list(
                    url, story_data_item['title'], site_key, total_chapters=total_chapters
                )
            ...
            await crawl_missing_chapters_for_story(
                site_key, session, chapters, story_data_item, current_discovery_genre_data,
                story_folder_path, crawl_state, num_batches=num_batches, state_file=state_file
            )
            files_now = len(get_saved_chapters_files(story_folder_path))
            if files_now >= total_chapters:
                logger.info(f"Đã crawl đủ chương {files_now}/{total_chapters} cho '{story_data_item['title']}' (từ nguồn {source.get('site_key')})")
                return
            crawled = True
    ...
    while True:
        try:
            stories, total_pages, crawled_pages = await adapter.get_all_stories_from_genre_with_page_check(
                genre_data['name'], genre_data['url'],site_key, MAX_STORIES_PER_GENRE_PAGE
            )  # type: ignore
...
async def retry_failed_genres(adapter, site_key, settings: WorkerSettings, shuffle_func):
    import json
    import random
    ...
    while True:
        ...
        if failed_genres:
            if round_idx < settings.retry_genre_round_limit:
                ...
                logger.error(f"Sleep {settings.retry_sleep_seconds // 60} phút rồi retry lại các genre fail: {genre_names}")
...
async def run_crawler(adapter, site_key, genres, settings: WorkerSettings):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        batches = split_batches(genres, max(1, (len(genres) + batch_size - 1) // batch_size))
...
async def run_single_site(site_key: str, env_overrides: Optional[Dict[str, str]] = None, crawl_mode: Optional[str] = None):
