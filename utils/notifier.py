...
class NotificationOptions:
    ...
    def resolve_parse_mode(self) -> Optional[str]:
        if self.parse_mode is None:
            return TELEGRAM_PARSE_MODE
        return self.parse_mode

    def resolve_disable_preview(self) -> Optional[bool]:
...
async def notify_progress(message: str, current: int, total: Optional[int] = None, **kwargs: Any):
